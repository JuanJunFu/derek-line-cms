...
const EVENT_LABELS: Record<string, { icon: string; label: string; bg: string; text: string }> = {
  STORE_CALL: { icon: "📞", label: "致電門市", bg: "bg-red-50", text: "text-red-600" },
  STORE_NAV:  { icon: "🗺️", label: "導航前往", bg: "bg-orange-50", text: "text-orange-600" },
  STORE_LINE: { icon: "💬", label: "門市LINE", bg: "bg-[var(--brand-accent)]/15", text: "text-[var(--brand-accent)]" },
  FOLLOW:     { icon: "✅", label: "加入好友", bg: "bg-emerald-50", text: "text-emerald-600" },
  PRODUCT_VIEW: { icon: "🛁", label: "瀏覽產品", bg: "bg-blue-50", text: "text-blue-600" },
  REGION_SELECT: { icon: "📍", label: "選擇地區", bg: "bg-emerald-50", text: "text-emerald-600" },
  MESSAGE:    { icon: "💬", label: "傳送訊息", bg: "bg-purple-50", text: "text-purple-600" },
  POSTBACK:   { icon: "👆", label: "點擊按鈕", bg: "bg-indigo-50", text: "text-indigo-600" },
};
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[var(--brand-accent)]/10 border border-[var(--brand-accent)]/20 rounded-xl p-4 text-center">
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">⚡ 24h 高意圖行動</p>
        </div>
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
          <p className="text-3xl font-bold text-emerald-600">{todayNewFollows}</p>
          <p className="text-xs text-[var(--text-muted)] mt-1">✅ 今日新加好友</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {hotProfiles.length === 0 ? (
              ...
              hotProfiles.map((p) => (
                <Link
                  ...
                  className="block bg-[var(--bg-secondary)] border border-red-200 rounded-lg p-3 hover:border-red-300 transition"
                >
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {p.customerType === "returning" && (
                          <span className="text-xs text-blue-600">🔄 老客戶</span>
                        )}
                    ...
                    <div className="text-right shrink-0 ml-2">
                      <p className="text-xs text-[var(--brand-accent)]">{p.relationshipLevel ?? "新識"}</p>
                      <p className="text-xs text-[var(--text-muted)]">{timeAgo(p.lastActive)}</p>
                    </div>
                  </div>
                  {/* Tags */}
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {p.tags.filter((t) => t.startsWith("Intent:") || t === "Status:High_Purchase_Intent").slice(0, 3).map((tag) => (
                      <span key={tag} className="text-xs px-1.5 py-0.5 rounded bg-red-50 text-red-600">
                        {INTENT_ZH[tag] ?? tag}
                      </span>
                    ))}
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            <h2 className="text-sm font-bold text-[var(--brand-accent)] mb-3">
              ⚡ 24h 高意圖行動（{recentHighIntent.length} 筆）
            </h2>
            <div className="space-y-1.5 max-h-48 overflow-y-auto">
              {recentHighIntent.length === 0 ? (
                <p className="text-sm text-[var(--text-muted)]">24 小時內無高意圖行動</p>
              ) : (
                recentHighIntent.map((e) => {
                  ...
                  return (
                    <Link
                      ...
                      className="flex items-center gap-2 bg-[var(--brand-accent)]/10 border border-[var(--brand-accent)]/20 rounded-lg px-3 py-2 hover:border-[var(--brand-accent)]/30 transition"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${ev?.text ?? "text-[var(--brand-accent)]"}`}>{ev?.label ?? e.eventType}</p>
                      </div>
                      <span className="text-xs text-[var(--text-muted)] shrink-0">{timeAgo(e.createdAt)}</span>
                    </Link>
                  );
                })
              )}
          ...
          {/* Live feed (1h) */}
          <div>
            <h2 className="text-sm font-bold text-[var(--text-secondary)] mb-3">
              📡 即時動態（最近 1 小時，{liveFeed.length} 筆）
            </h2>
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {liveFeed.length === 0 ? (
                <p className="text-sm text-[var(--text-muted)]">最近 1 小時無事件</p>
              ) : (
                liveFeed.map((e) => {
                  const ev = EVENT_LABELS[e.eventType];
                  ...
                  return (
                    <Link
                      key={e.id}
                      href={`/leads/${encodeURIComponent(e.userId)}`}
                      className={`flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-[var(--bg-tertiary)] transition ${
                        isHot ? "bg-red-50" : "bg-[var(--bg-secondary)]/50"
                      }`}
                    >
                      ...
                      <span className="text-xs text-[var(--text-secondary)] flex-1 truncate">
                        ...
                        <span className="text-[var(--text-muted)]"> · {ev?.label ?? e.eventType}</span>
                      </span>
                      {isHot && <span className="text-xs text-red-600">🔥</span>}
                      <span className="text-xs text-[var(--text-muted)] shrink-0">{minutesAgo(e.createdAt)}</span>
                    </Link>
                  );
                })
              )}
